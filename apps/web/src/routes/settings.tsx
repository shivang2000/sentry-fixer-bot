...
function wsUrlFor(provider: "claude" | "github"): string {
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-3">
        ...
        {active ? (
          <LoginSession key={active} provider={active} onDone={() => setActive(null)} />
        ) : null}
      </CardContent>
    </Card>
  );
...
function LoginSession({ provider, onDone }: { provider: "claude" | "github"; onDone: () => void }) {
    ...
    ws.onmessage = (evt) => {
      ...
      switch (msg.type) {
        ...
        case "exit":
          ...
          if (msg.code === 0) toast.success(`${provider} login complete`);
          break;
        case "error":
