#!/usr/bin/env bash
# setup.sh — one-shot bootstrap for alertforge.
#
# Auto-detects environment and switches behavior:
#   * ALERTFORGE_RUN_MODE=ec2       — installs deps via dnf, starts Postgres in docker,
#                              installs systemd units, configures nginx on :80
#                              (suitable for a fresh Amazon Linux 2023 EC2)
#   * ALERTFORGE_RUN_MODE=container — assumes deps are pre-installed (from a Dockerfile),
...
REPO_URL="${ALERTFORGE_REPO_URL:-https://github.com/shivang2000/alertforge.git}"
REPO_BRANCH="${ALERTFORGE_REPO_BRANCH:-main}"
APP_DIR="${ALERTFORGE_APP_DIR:-/opt/alertforge/current}"
ENV_FILE="${ALERTFORGE_ENV_FILE:-/etc/alertforge/env}"
WORK_DIR="${ALERTFORGE_WORK_DIR:-/var/lib/alertforge}"
SERVER_PORT="${ALERTFORGE_SERVER_PORT:-3000}"
HTTP_PORT="${ALERTFORGE_HTTP_PORT:-80}"
...
run_as_alertforge() {
  priv runuser -u alertforge-runner -- bash -lc "$1"
}

# Mode detection: prefer explicit override, fall back to auto.
ALERTFORGE_RUN_MODE="${ALERTFORGE_RUN_MODE:-auto}"
if [[ "$ALERTFORGE_RUN_MODE" == "auto" ]]; then
  if [[ -f /.dockerenv ]] || grep -qE 'docker|containerd|podman|kubepods' /proc/1/cgroup 2>/dev/null; then
    ALERTFORGE_RUN_MODE=container
  else
    ALERTFORGE_RUN_MODE=ec2
  fi
fi
log "run mode: $ALERTFORGE_RUN_MODE"

# Container mode pins every piece of mutable per-instance state onto one
# named docker volume mounted at ALERTFORGE_STATE_DIR (default /alertforge/state). That
# means the env file, alertforge-runner's HOME (claude/gh/codex/opencloud dotfiles,
# npm cache), the per-session work dir, and skill installs all live on the
# same disk artefact — one mount survives `docker compose restart` AND a
# container recreate. EC2 mode keeps the original FHS paths.
if [[ "$ALERTFORGE_RUN_MODE" == "container" ]]; then
  ALERTFORGE_STATE_DIR="${ALERTFORGE_STATE_DIR:-/alertforge/state}"
  ENV_FILE="${ALERTFORGE_ENV_FILE:-$ALERTFORGE_STATE_DIR/etc/env}"
  WORK_DIR="${ALERTFORGE_WORK_DIR:-$ALERTFORGE_STATE_DIR/work}"
  ALERTFORGE_HOME="$ALERTFORGE_STATE_DIR/home"
fi
...
# ---------------------------------------------------------------------------
if [[ "$ALERTFORGE_RUN_MODE" == "ec2" ]]; then
  ...
  if [[ -n "$LOCAL_CHECKOUT" ]] && [[ -x "$LOCAL_CHECKOUT/deploy/ec2/bootstrap-al2023.sh" ]]; then
    ...
    TMP_BOOT="$(mktemp)"
    curl -fsSL \
      "https://raw.githubusercontent.com/shivang2000/alertforge/${REPO_BRANCH}/deploy/ec2/bootstrap-al2023.sh" \
      -o "$TMP_BOOT"
    priv bash "$TMP_BOOT"
    rm -f "$TMP_BOOT"
  fi
...
if [[ "$ALERTFORGE_RUN_MODE" == "container" ]]; then
  ...
  if id -u alertforge-runner >/dev/null 2>&1 && [[ "$(stat -c '%U' "$APP_DIR")" == "alertforge-runner" ]]; then
    run_as_alertforge "cd '$APP_DIR' && git fetch --quiet origin && git reset --quiet --hard 'origin/${REPO_BRANCH}'"
  else
...
# Make sure alertforge-runner can read the repo (EC2 mode).
if [[ "$ALERTFORGE_RUN_MODE" == "ec2" ]]; then
  priv chown -R alertforge-runner:alertforge-runner "$APP_DIR" 2>/dev/null || true
fi

# ---------------------------------------------------------------------------
# 3. Generate (or refresh) /etc/alertforge/env
# ---------------------------------------------------------------------------
...
if [[ "$ALERTFORGE_RUN_MODE" == "container" ]]; then
  ...
  DB_URL="postgres://postgres:password@127.0.0.1:5433/alertforge"
...
if [[ "$ALERTFORGE_RUN_MODE" == "container" ]]; then
  # Container mode: settings UI (running as alertforge-runner) needs to atomically
  ...
  priv install -d -m 0770 -o root -g alertforge-runner "$(dirname "$ENV_FILE")" 2>/dev/null || \
    priv install -d -m 0775 "$(dirname "$ENV_FILE")"
else
  priv install -d -m 0750 -o root -g alertforge-runner "$(dirname "$ENV_FILE")" 2>/dev/null || \
    priv install -d -m 0755 "$(dirname "$ENV_FILE")"
fi

# In container mode the env file lives on a named docker volume
# (alertforge_dev_etc) so it survives container restarts AND captures every
...
if [[ "$ALERTFORGE_RUN_MODE" == "container" ]] && [[ -s "$ENV_FILE" ]] && \
   priv grep -q '^DATABASE_URL=' "$ENV_FILE" 2>/dev/null; then
  log "  env file already populated; preserving (down -v to reset)"
else

# Container vs EC2: in container mode WORK_DIR is already a final path
# (/alertforge/state/work) and HOME must point at the state volume so every CLI's
# dotfiles (claude, gh, codex, opencloud) persist. EC2 mode keeps the
# legacy /var/lib/alertforge/work layout and inherits the default /home/alertforge-runner.
if [[ "$ALERTFORGE_RUN_MODE" == "container" ]]; then
  WORK_DIR_FOR_ENV_FILE="${WORK_DIR}"
  HOME_LINE_FOR_ENV_FILE="HOME=${ALERTFORGE_HOME}"
else
  WORK_DIR_FOR_ENV_FILE="${WORK_DIR}/work"
  HOME_LINE_FOR_ENV_FILE=""
fi

priv tee "${ENV_FILE}.tmp" >/dev/null <<EOF
# Generated by setup.sh on $(date -u +%FT%TZ)
# Mode: ${ALERTFORGE_RUN_MODE}
...
# --- Agent runtime ---
WORK_DIR=${WORK_DIR_FOR_ENV_FILE}
...
EOF
if [[ "$ALERTFORGE_RUN_MODE" == "container" ]]; then
  # Container mode: secret-writer (running as alertforge-runner) must overwrite this
  # file from /settings. 0660 + root:alertforge-runner gives it group write.
  priv chmod 0660 "${ENV_FILE}.tmp"
else
  priv chmod 0640 "${ENV_FILE}.tmp"
fi
priv chown root:alertforge-runner "${ENV_FILE}.tmp" 2>/dev/null || true
...
if [[ "$ALERTFORGE_RUN_MODE" == "ec2" ]]; then
  ...
  for i in $(seq 1 60); do
    if priv docker inspect --format='{{.State.Health.Status}}' alertforge-postgres 2>/dev/null | grep -q healthy; then
      log "  Postgres ready"
      break
    fi
    sleep 2
    if [[ "$i" -eq 60 ]]; then
      ...
      priv docker logs --tail 30 alertforge-postgres || true
      exit 1
    fi
  done
else
  ...
  for i in $(seq 1 60); do
    if run_as_alertforge "cd '$APP_DIR' && bun run -e 'await Bun.sql\`select 1\`.simple()' 2>/dev/null" >/dev/null 2>&1; then
...
run_as_alertforge "cd '$APP_DIR' && bun install --frozen-lockfile"

log "  applying database migrations"
run_as_alertforge \
  "cd '$APP_DIR' && set -a && . '$ENV_FILE' && set +a && bun --filter=@alertforge/db db:migrate"
...
if [[ "$ALERTFORGE_RUN_MODE" == "ec2" ]]; then
  ...
  for unit in alertforge-server.service alertforge-worker.service alertforge-backup.service alertforge-backup.timer; do
  ...
  priv systemctl enable alertforge-server.service alertforge-worker.service
  priv systemctl restart alertforge-server.service alertforge-worker.service
  if [[ -f /etc/systemd/system/alertforge-backup.timer ]]; then
    priv systemctl enable --now alertforge-backup.timer || true
  fi
else
...
if [[ "$ALERTFORGE_RUN_MODE" == "ec2" ]]; then
  ...
  priv tee /etc/nginx/conf.d/alertforge.conf >/dev/null <<NGINX
upstream alertforge_server {
...
server {
  ...
  location /api/chat/ {
    proxy_pass         http://alertforge_server;
...
if [[ "$ALERTFORGE_RUN_MODE" == "ec2" ]]; then
  ...
  for i in $(seq 1 20); do
    ...
    sleep 1
    if [[ "$i" -eq 20 ]]; then
      err "/healthz did not respond. Check: sudo journalctl -fu alertforge-server"
    fi
  done
...
Service logs : sudo journalctl -fu alertforge-server | sudo journalctl -fu alertforge-worker
Operator runbook : ${APP_DIR}/docs/runbook.md

DONE
else
  log "step 8/8: launching alertforge-server in foreground (container mode)"
  cat <<DONE

\033[32m✓ Container ready. Starting alertforge-server now…\033[0m
...
DONE
  ...
  cd "$APP_DIR"
  exec runuser -u alertforge-runner -- bash -lc \
