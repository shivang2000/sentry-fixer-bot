...
if [[ -n "$LOCAL_CHECKOUT" ]] && [[ "$LOCAL_CHECKOUT" != "$APP_DIR" ]]; then
  if [[ "$SFB_RUN_MODE" == "container" ]]; then
    # In container mode we expect the repo to already be at $APP_DIR
    # (mounted or COPY'd by the Dockerfile). Symlink only if needed.
    if [[ ! -d "$APP_DIR" ]]; then
      priv ln -s "$LOCAL_CHECKOUT" "$APP_DIR"
    fi
  else
    priv rm -rf "$APP_DIR"
...
# Make sure sfb-runner can read the repo.
