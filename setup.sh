...
# named docker volume mounted at SFB_STATE_DIR (default /sfb/state). That
# means the env file, sfb-runner's HOME (claude/gh/codex/opencloud dotfiles,
...
if [[ "$SFB_RUN_MODE" == "container" ]]; then
  SFB_STATE_DIR="${SFB_STATE_DIR:-/sfb/state}"
  ENV_FILE="${SFB_ENV_FILE:-$SFB_STATE_DIR/etc/env}"
  WORK_DIR="${SFB_WORK_DIR:-$SFB_STATE_DIR/work}"
  SFB_HOME="$SFB_STATE_DIR/home"
fi
...
# (/sfb/state/work) and HOME must point at the state volume so every CLI's
# dotfiles (claude, gh, codex, opencloud) persist. EC2 mode keeps the
# legacy /var/lib/sfb/work layout and inherits the default /home/sfb-runner.
if [[ "$SFB_RUN_MODE" == "container" ]]; then
  WORK_DIR_FOR_ENV_FILE="${WORK_DIR}"
  HOME_LINE_FOR_ENV_FILE="HOME=${SFB_HOME}"
else
